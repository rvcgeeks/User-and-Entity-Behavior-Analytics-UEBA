...
def train(files_train,predict_label):
    ...
    y_pred_1[y_pred_1 == 1] = 1  # Inliers
    y_pred[y_pred == -1] = 1  # Outliers
    y_pred_1[y_pred_1 == -1] = 0  # Outliers
    labels = np.transpose(np.vstack((y_pred, y_pred_1)))
    np.savetxt(predict_label,labels,fmt='%d',delimiter=',')
...
def figure_ponit(user):
    ...
    y2=[]
    z2=[]
    all_data_in=working_folder+'Mix_loss.csv'
    all_label_in=working_folder+'label_loss.csv'
...
if __name__ == "__main__":
    ...
    for user,subconfig in CONFIG['monitor'].items():
        ...
        files_train=working_folder+'Mix_loss.csv'
        predict_label=working_folder+'label_loss.csv'
        ...
        figure_ponit(user)
    #At this point analyst will review the actions that occured on the day corresponding to anomalous days
    #mentioned in label_loss.csv for TPs and FPs and correct the csv file.
